...
typedef uint8_t (*mem_callback)(bool write, uint16_t addr, uint8_t inval);
...
typedef struct {
    ...
    mem_page layers[MEM_NUM_LAYERS][MEM_NUM_PAGES];
    /* the pages that are actually visible to the emulated CPU */
    mem_page page_table[MEM_NUM_PAGES];
...
extern void mem_init(memory* mem);
/* map a range of RAM */
extern void mem_map_ram(memory* mem, int layer, uint16_t addr, uint32_t size, uint8_t* ptr);
/* map a range of ROM */
extern void mem_map_rom(memory* mem, int layer, uint16_t addr, uint32_t size, const uint8_t* ptr);
/* map a range of memory to different read/write pointers (e.g. for RAM behind ROM) */
extern void mem_map_rw(memory* mem, int layer, uint16_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr);
/* map a range of memory to a memory-mapped-io callback function */
extern void mem_map_io(memory* mem, int layer, uint16_t addr, uint32_t size, mem_callback cb);
/* unmap all memory pages in a layer, also updates the CPU-visible page-table */
extern void mem_unmap_layer(memory* mem, int layer);
/* unmap all memory pages in all layers, also updates the CPU-visible page-table */
extern void mem_unmap_all(memory* mem);
/* get the host-memory read-ptr of a emulator memory address */
extern const uint8_t* mem_readptr(memory* mem, uint16_t addr);
...
static inline uint8_t mem_rdio(memory* mem, uint16_t addr) {
    ...
    if (page->write_ptr) {
        ...
        return page->mem_cb(false, addr, 0);
    }
}
/* write a byte at address without memory-mapped-io support */
static inline void mem_wr(memory* mem, uint16_t addr, uint8_t data) {
...
static inline void mem_wrio(memory* mem, uint16_t addr, uint8_t data) {
    mem_page* page = &mem->page_table[addr>>MEM_PAGE_SHIFT];
    if (page->write_ptr) {
        /* a regular memory page */
...
static void _mem_update_page_table(memory* m, int page_index) {
...
static void _mem_map(memory* m, int layer, uint16_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr, mem_callback cb) {
    CHIPS_ASSERT(m);
    ...
    for (int i = 0; i < num; i++) {
        ...
        mem_page* page = &m->layers[layer][page_index];
        if (cb) {
...
void mem_map_ram(memory* m, int layer, uint16_t addr, uint32_t size, uint8_t* ptr) {
    CHIPS_ASSERT(ptr);
    _mem_map(m, layer, addr, size, ptr, ptr, 0);
}

void mem_map_rom(memory* m, int layer, uint16_t addr, uint32_t size, const uint8_t* ptr) {
    CHIPS_ASSERT(ptr);
    _mem_map(m, layer, addr, size, ptr, 0, 0);
}

void mem_map_rw(memory* m, int layer, uint16_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr) {
    CHIPS_ASSERT(read_ptr && write_ptr);
    _mem_map(m, layer, addr, size, read_ptr, write_ptr, 0);
}

void mem_map_io(memory* m, int layer, uint16_t addr, uint32_t size, mem_callback cb) {
    CHIPS_ASSERT(cb);
    _mem_map(m, layer, addr, size, 0, 0, cb);
}

void mem_unmap_layer(memory* m, int layer) {
    CHIPS_ASSERT(m);
    CHIPS_ASSERT((layer >= 0) && (layer < MEM_NUM_LAYERS));
...
void mem_unmap_all(memory* m) {
    for (int layer_index = 0; layer_index < MEM_NUM_LAYERS; layer_index++) {
        for (int page_index = 0; page_index < MEM_NUM_PAGES; page_index++) {
            mem_page* page = &m->layers[layer_index][page_index];
            page->read_ptr = 0; 
            page->write_ptr = 0;
        }
    }
...
const uint8_t* mem_readptr(memory* m, uint16_t addr) {
    CHIPS_ASSERT(m);
    return &(m->page_table[addr>>MEM_PAGE_SHIFT].read_ptr[addr&MEM_PAGE_MASK]);
} 

void mem_write_range(memory* m, uint16_t addr, const uint8_t* src, int num_bytes) {
