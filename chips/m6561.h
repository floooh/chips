...
void m6561_init(m6561_t* vic, const m6561_desc_t* desc) {
    ...
    CHIPS_ASSERT((0 == desc->framebuffer.ptr) || (desc->framebuffer.size >= (_M6561_HTOTAL*8*_M6561_VTOTAL*sizeof(uint32_t))));
...
chips_rect_t m6561_screen(m6561_t* vic) {
    CHIPS_ASSERT(vic);
    return (chips_rect_t){
        .x = vic->debug_vis ? 0 : vic->crt.vis_x0 * _M6561_PIXELS_PER_TICK,
        .y = vic->debug_vis ? 0 : vic->crt.vis_y0,
