/* machine generated, don't edit! */
/* set 16-bit address in 64-bit pin mask*/
#define _SA(addr) pins=(pins&~0xFFFF)|((addr)&0xFFFFULL)
/* set 16-bit address and 8-bit data in 64-bit pin mask */
#define _SAD(addr,data) pins=(pins&~0xFFFFFF)|(((data)<<16)&0xFF0000ULL)|((addr)&0xFFFFULL)
/* extract 8-bit data from 64-bit pin mask */
#define _GD() ((uint8_t)((pins&0xFF0000ULL)>>16))
/* enable control pins */
#define _ON(m) pins|=(m)
/* disable control pins */
#define _OFF(m) pins&=~(m)
/* execute a tick */
#define _T() pins=tick(pins);ticks++
/* a memory read tick */
#define _RD() _ON(M6502_RW);_T();
/* a memory write tick */
#define _WR() _OFF(M6502_RW);_T()
/* implied addressing mode, this still puts the PC on the address bus */
#define _A_IMP() _SA(c.PC)
/* immediate addressing mode */
#define _A_IMM() _SA(c.PC++)
/* zero-page addressing mode */
#define _A_ZER() _SA(c.PC++);_RD();a=_GD();_SA(a)
/* zero page + X addressing mode */
#define _A_ZPX() _SA(c.PC++);_RD();a=_GD();_SA(a);_RD();a=(a+c.X)&0x00FF;_SA(a)
/* zero page + Y addressing mode */
#define _A_ZPY() _SA(c.PC++);_RD();a=_GD();_SA(a);_RD();a=(a+c.Y)&0x00FF;_SA(a)
/* absolute addressing mode */
#define _A_ABS() _SA(c.PC++);_RD();l=_GD();_SA(c.PC++);_RD();h=_GD();a=(h<<8)|l;_SA(a)
/* absolute+X addressing mode for read-only instructions, early out if no page boundary is crossed */
#define _A_ABX_R() _SA(c.PC++);_RD();t=_GD()+c.X;_SA(c.PC++);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);if((t&0xFF00)!=0){_RD();a=(a&0xFF00)+t;_SA(a);}
/* absolute+X addressing mode for read/write instructions */
#define _A_ABX_W() _SA(c.PC++);_RD();t=_GD()+c.X;_SA(c.PC++);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);_RD();a=(a&0xFF00)+t;_SA(a)
/* absolute+Y addressing mode for read-only instructions, early out if no page boundary is crossed */
#define _A_ABY_R() _SA(c.PC++);_RD();t=_GD()+c.Y;_SA(c.PC++);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);if((t&0xFF00)!=0){_RD();a=(a&0xFF00)+t;_SA(a);}
/* absolute+Y addressing mode for read/write instructions */
#define _A_ABY_W() _SA(c.PC++);_RD();t=_GD()+c.Y;_SA(c.PC++);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);_RD();a=(a&0xFF00)+t;_SA(a)
/* (zp,X) indexed indirect addressing mode */
#define _A_IDX() _SA(c.PC++);_RD();a=_GD();_SA(a);_RD();a=(a+c.X)&0xFF;_SA(a);_RD();t=_GD();a=(a+1)&0xFF;_SA(a);_RD();a=(_GD()<<8)|t;_SA(a);
/* (zp),Y indirect indexed addressing mode for read-only instructions, early out if no page boundary crossed */
#define _A_IDY_R() _SA(c.PC++);_RD();a=_GD();_SA(a);_RD();t=_GD()+c.Y;a=(a+1)&0xFF;_SA(a);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);if((t&0xFF00)!=0){_RD();a=(a&0xFF00)+t;_SA(a);}
/* (zp),Y indirect indexed addressing mode for read/write instructions */
#define _A_IDY_W() _SA(c.PC++);_RD();a=_GD();_SA(a);_RD();t=_GD()+c.Y;a=(a+1)&0xFF;_SA(a);_RD();a=(_GD()<<8)|(t&0xFF);_SA(a);_RD();a=(a&0xFF00)+t;_SA(a)

uint32_t m6502_exec(m6502_t* cpu, uint32_t num_ticks) {
  m6502_t c = *cpu;
  uint8_t l, h;
  uint16_t a, t;
  uint32_t ticks = 0;
  uint64_t pins = c.PINS;
  const m6502_tick_t tick = c.tick;
  do {
    /* fetch opcode */
    _SA(c.PC++);_ON(M6502_SYNC);_RD();_OFF(M6502_SYNC);
    const uint8_t opcode = _GD();
    switch (opcode) {
      case 0x0:/*BRK */_A_IMP();/*FIXME*/break;
      case 0x1:/*ORA (zp,X)*/_A_IDX();/* FIXME */break;
      case 0x2:/*ASL *//* FIXME */break;
      case 0x3:/*SLO (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0x4:/*NOP zp (undoc)*/_A_ZER();_RD();break;
      case 0x5:/*ORA zp*/_A_ZER();/* FIXME */break;
      case 0x6:/*ASL zp*/_A_ZER();/* FIXME */break;
      case 0x7:/*SLO zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0x8:/*PHP */_A_IMP();/* FIXME */break;
      case 0x9:/*ORA #*/_A_IMM();/* FIXME */break;
      case 0xa:/*ASLA */_A_IMP();/* FIXME */break;
      case 0xb:/*SLO  (undoc)*//* FIXME */break;
      case 0xc:/*NOP abs (undoc)*/_A_ABS();_RD();break;
      case 0xd:/*ORA abs*/_A_ABS();/* FIXME */break;
      case 0xe:/*ASL abs*/_A_ABS();/* FIXME */break;
      case 0xf:/*SLO abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0x10:/*BPL #*/_A_IMM();/* FIXME */break;
      case 0x11:/*ORA (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0x12:/*ASL *//* FIXME */break;
      case 0x13:/*SLO (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0x14:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0x15:/*ORA zp,X*/_A_ZPX();/* FIXME */break;
      case 0x16:/*ASL zp,X*/_A_ZPX();/* FIXME */break;
      case 0x17:/*SLO zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0x18:/*CL CF */_A_IMP();/* FIXME */break;
      case 0x19:/*ORA abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0x1a:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0x1b:/*SLO abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0x1c:/*NOP abs,X (undoc)*/_A_ABX_R();_RD();break;
      case 0x1d:/*ORA abs,X*/_A_ABX_R();/* FIXME */break;
      case 0x1e:/*ASL abs,X*/_A_ABX_W();/* FIXME */break;
      case 0x1f:/*SLO abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
      case 0x20:/*JSR */_SA(c.PC++);/* FIXME */break;
      case 0x21:/*AND (zp,X)*/_A_IDX();/* FIXME */break;
      case 0x22:/*ROL *//* FIXME */break;
      case 0x23:/*RLA (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0x24:/*BIT zp*/_A_ZER();/* FIXME */break;
      case 0x25:/*AND zp*/_A_ZER();/* FIXME */break;
      case 0x26:/*ROL zp*/_A_ZER();/* FIXME */break;
      case 0x27:/*RLA zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0x28:/*PLP */_A_IMP();/* FIXME */break;
      case 0x29:/*AND #*/_A_IMM();/* FIXME */break;
      case 0x2a:/*ROLA */_A_IMP();/* FIXME */break;
      case 0x2b:/*RLA  (undoc)*//* FIXME */break;
      case 0x2c:/*BIT abs*/_A_ABS();/* FIXME */break;
      case 0x2d:/*AND abs*/_A_ABS();/* FIXME */break;
      case 0x2e:/*ROL abs*/_A_ABS();/* FIXME */break;
      case 0x2f:/*RLA abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0x30:/*BMI #*/_A_IMM();/* FIXME */break;
      case 0x31:/*AND (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0x32:/*ROL *//* FIXME */break;
      case 0x33:/*RLA (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0x34:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0x35:/*AND zp,X*/_A_ZPX();/* FIXME */break;
      case 0x36:/*ROL zp,X*/_A_ZPX();/* FIXME */break;
      case 0x37:/*RLA zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0x38:/*SE CF */_A_IMP();/* FIXME */break;
      case 0x39:/*AND abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0x3a:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0x3b:/*RLA abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0x3c:/*NOP abs,X (undoc)*/_A_ABX_R();_RD();break;
      case 0x3d:/*AND abs,X*/_A_ABX_R();/* FIXME */break;
      case 0x3e:/*ROL abs,X*/_A_ABX_W();/* FIXME */break;
      case 0x3f:/*RLA abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
      case 0x40:/*RTI */_A_IMP();/* FIXME */break;
      case 0x41:/*EOR (zp,X)*/_A_IDX();/* FIXME */break;
      case 0x42:/*LSR *//* FIXME */break;
      case 0x43:/*SRE (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0x44:/*NOP zp (undoc)*/_A_ZER();_RD();break;
      case 0x45:/*EOR zp*/_A_ZER();/* FIXME */break;
      case 0x46:/*LSR zp*/_A_ZER();/* FIXME */break;
      case 0x47:/*SRE zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0x48:/*PHA */_A_IMP();/* FIXME */break;
      case 0x49:/*EOR #*/_A_IMM();/* FIXME */break;
      case 0x4a:/*LSRA */_A_IMP();/* FIXME */break;
      case 0x4b:/*SRE  (undoc)*//* FIXME */break;
      case 0x4c:/*JMP */_SA(c.PC++);_RD();a=_GD();_SA(c.PC++);/* FIXME */break;
      case 0x4d:/*EOR abs*/_A_ABS();/* FIXME */break;
      case 0x4e:/*LSR abs*/_A_ABS();/* FIXME */break;
      case 0x4f:/*SRE abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0x50:/*BVC #*/_A_IMM();/* FIXME */break;
      case 0x51:/*EOR (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0x52:/*LSR *//* FIXME */break;
      case 0x53:/*SRE (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0x54:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0x55:/*EOR zp,X*/_A_ZPX();/* FIXME */break;
      case 0x56:/*LSR zp,X*/_A_ZPX();/* FIXME */break;
      case 0x57:/*SRE zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0x58:/*CL IF */_A_IMP();/* FIXME */break;
      case 0x59:/*EOR abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0x5a:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0x5b:/*SRE abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0x5c:/*NOP abs (undoc)*/_A_ABS();_RD();break;
      case 0x5d:/*EOR abs,X*/_A_ABX_R();/* FIXME */break;
      case 0x5e:/*LSR abs,X*/_A_ABX_W();/* FIXME */break;
      case 0x5f:/*SRE abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
      case 0x60:/*RTS */_A_IMP();/* FIXME */break;
      case 0x61:/*ADC (zp,X)*/_A_IDX();/* FIXME */break;
      case 0x62:/*ROR *//* FIXME */break;
      case 0x63:/*RRA (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0x64:/*NOP zp (undoc)*/_A_ZER();_RD();break;
      case 0x65:/*ADC zp*/_A_ZER();/* FIXME */break;
      case 0x66:/*ROR zp*/_A_ZER();/* FIXME */break;
      case 0x67:/*RRA zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0x68:/*PLA */_A_IMP();/* FIXME */break;
      case 0x69:/*ADC #*/_A_IMM();/* FIXME */break;
      case 0x6a:/*RORA */_A_IMP();/* FIXME */break;
      case 0x6b:/*RRA  (undoc)*//* FIXME */break;
      case 0x6c:/*JMPI */_SA(c.PC++);_RD();a=_GD();_SA(c.PC++);/* FIXME */break;
      case 0x6d:/*ADC abs*/_A_ABS();/* FIXME */break;
      case 0x6e:/*ROR abs*/_A_ABS();/* FIXME */break;
      case 0x6f:/*RRA abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0x70:/*BVS #*/_A_IMM();/* FIXME */break;
      case 0x71:/*ADC (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0x72:/*ROR *//* FIXME */break;
      case 0x73:/*RRA (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0x74:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0x75:/*ADC zp,X*/_A_ZPX();/* FIXME */break;
      case 0x76:/*ROR zp,X*/_A_ZPX();/* FIXME */break;
      case 0x77:/*RRA zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0x78:/*SE IF */_A_IMP();/* FIXME */break;
      case 0x79:/*ADC abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0x7a:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0x7b:/*RRA abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0x7c:/*NOP abs (undoc)*/_A_ABS();_RD();break;
      case 0x7d:/*ADC abs,X*/_A_ABX_R();/* FIXME */break;
      case 0x7e:/*ROR abs,X*/_A_ABX_W();/* FIXME */break;
      case 0x7f:/*RRA abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
      case 0x80:/*NOP # (undoc)*/_A_IMM();_RD();break;
      case 0x81:/*STA (zp,X)*/_A_IDX();/* FIXME */break;
      case 0x82:/*NOP # (undoc)*/_A_IMM();_RD();break;
      case 0x83:/*SAX (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0x84:/*STY zp*/_A_ZER();/* FIXME */break;
      case 0x85:/*STA zp*/_A_ZER();/* FIXME */break;
      case 0x86:/*STX zp*/_A_ZER();/* FIXME */break;
      case 0x87:/*SAX zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0x88:/*DEY */_A_IMP();/* FIXME */break;
      case 0x89:/*NOP # (undoc)*/_A_IMM();_RD();break;
      case 0x8a:/*TXA */_A_IMP();/* FIXME */break;
      case 0x8b:/*SAX  (undoc)*//* FIXME */break;
      case 0x8c:/*STY abs*/_A_ABS();/* FIXME */break;
      case 0x8d:/*STA abs*/_A_ABS();/* FIXME */break;
      case 0x8e:/*STX abs*/_A_ABS();/* FIXME */break;
      case 0x8f:/*SAX abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0x90:/*BCC #*/_A_IMM();/* FIXME */break;
      case 0x91:/*STA (zp),Y*/_A_IDY_W();/* FIXME */break;
      case 0x92:/*STX *//* FIXME */break;
      case 0x93:/*SAX  (undoc)*//* FIXME */break;
      case 0x94:/*STY zp,X*/_A_ZPX();/* FIXME */break;
      case 0x95:/*STA zp,X*/_A_ZPX();/* FIXME */break;
      case 0x96:/*STX zp,Y*/_A_ZPY();/* FIXME */break;
      case 0x97:/*SAX zp,Y (undoc)*/_A_ZPY();/* FIXME */break;
      case 0x98:/*TYA */_A_IMP();/* FIXME */break;
      case 0x99:/*STA abs,Y*/_A_ABY_W();/* FIXME */break;
      case 0x9a:/*TXS */_A_IMP();/* FIXME */break;
      case 0x9b:/*SAX  (undoc)*//* FIXME */break;
      case 0x9c:/*STY *//* FIXME */break;
      case 0x9d:/*STA abs,X*/_A_ABX_W();/* FIXME */break;
      case 0x9e:/*STX *//* FIXME */break;
      case 0x9f:/*SAX  (undoc)*//* FIXME */break;
      case 0xa0:/*LDY #*/_A_IMM();/* FIXME */break;
      case 0xa1:/*LDA (zp,X)*/_A_IDX();/* FIXME */break;
      case 0xa2:/*LDX #*/_A_IMM();/* FIXME */break;
      case 0xa3:/*LAX (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0xa4:/*LDY zp*/_A_ZER();/* FIXME */break;
      case 0xa5:/*LDA zp*/_A_ZER();/* FIXME */break;
      case 0xa6:/*LDX zp*/_A_ZER();/* FIXME */break;
      case 0xa7:/*LAX zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0xa8:/*TAY */_A_IMP();/* FIXME */break;
      case 0xa9:/*LDA #*/_A_IMM();/* FIXME */break;
      case 0xaa:/*TAX */_A_IMP();/* FIXME */break;
      case 0xab:/*LAX  (undoc)*//* FIXME */break;
      case 0xac:/*LDY abs*/_A_ABS();/* FIXME */break;
      case 0xad:/*LDA abs*/_A_ABS();/* FIXME */break;
      case 0xae:/*LDX abs*/_A_ABS();/* FIXME */break;
      case 0xaf:/*LAX abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0xb0:/*BCS #*/_A_IMM();/* FIXME */break;
      case 0xb1:/*LDA (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0xb2:/*LDX *//* FIXME */break;
      case 0xb3:/*LAX (zp),Y (undoc)*/_A_IDY_R();/* FIXME */break;
      case 0xb4:/*LDY zp,X*/_A_ZPX();/* FIXME */break;
      case 0xb5:/*LDA zp,X*/_A_ZPX();/* FIXME */break;
      case 0xb6:/*LDX zp,Y*/_A_ZPY();/* FIXME */break;
      case 0xb7:/*LAX zp,Y (undoc)*/_A_ZPY();/* FIXME */break;
      case 0xb8:/*CL VF */_A_IMP();/* FIXME */break;
      case 0xb9:/*LDA abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0xba:/*TSX */_A_IMP();/* FIXME */break;
      case 0xbb:/*LAX  (undoc)*//* FIXME */break;
      case 0xbc:/*LDY abs,X*/_A_ABX_R();/* FIXME */break;
      case 0xbd:/*LDA abs,X*/_A_ABX_R();/* FIXME */break;
      case 0xbe:/*LDX abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0xbf:/*LAX abs,Y (undoc)*/_A_ABY_R();/* FIXME */break;
      case 0xc0:/*CPY #*/_A_IMM();/* FIXME */break;
      case 0xc1:/*CMP (zp,X)*/_A_IDX();/* FIXME */break;
      case 0xc2:/*NOP # (undoc)*/_A_IMM();_RD();break;
      case 0xc3:/*DCP (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0xc4:/*CPY zp*/_A_ZER();/* FIXME */break;
      case 0xc5:/*CMP zp*/_A_ZER();/* FIXME */break;
      case 0xc6:/*DEC zp*/_A_ZER();/* FIXME */break;
      case 0xc7:/*DCP zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0xc8:/*INY */_A_IMP();/* FIXME */break;
      case 0xc9:/*CMP #*/_A_IMM();/* FIXME */break;
      case 0xca:/*DEX */_A_IMP();/* FIXME */break;
      case 0xcb:/*DCP  (undoc)*//* FIXME */break;
      case 0xcc:/*CPY abs*/_A_ABS();/* FIXME */break;
      case 0xcd:/*CMP abs*/_A_ABS();/* FIXME */break;
      case 0xce:/*DEC abs*/_A_ABS();/* FIXME */break;
      case 0xcf:/*DCP abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0xd0:/*BNE #*/_A_IMM();/* FIXME */break;
      case 0xd1:/*CMP (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0xd2:/*DEC *//* FIXME */break;
      case 0xd3:/*DCP (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0xd4:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0xd5:/*CMP zp,X*/_A_ZPX();/* FIXME */break;
      case 0xd6:/*DEC zp,X*/_A_ZPX();/* FIXME */break;
      case 0xd7:/*DCP zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0xd8:/*CL DF */_A_IMP();/* FIXME */break;
      case 0xd9:/*CMP abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0xda:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0xdb:/*DCP abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0xdc:/*NOP abs,X (undoc)*/_A_ABX_R();_RD();break;
      case 0xdd:/*CMP abs,X*/_A_ABX_R();/* FIXME */break;
      case 0xde:/*DEC abs,X*/_A_ABX_W();/* FIXME */break;
      case 0xdf:/*DCP abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
      case 0xe0:/*CPX #*/_A_IMM();/* FIXME */break;
      case 0xe1:/*SBC (zp,X)*/_A_IDX();/* FIXME */break;
      case 0xe2:/*NOP # (undoc)*/_A_IMM();_RD();break;
      case 0xe3:/*ISB (zp,X) (undoc)*/_A_IDX();/* FIXME */break;
      case 0xe4:/*CPX zp*/_A_ZER();/* FIXME */break;
      case 0xe5:/*SBC zp*/_A_ZER();/* FIXME */break;
      case 0xe6:/*INC zp*/_A_ZER();/* FIXME */break;
      case 0xe7:/*ISB zp (undoc)*/_A_ZER();/* FIXME */break;
      case 0xe8:/*INX */_A_IMP();/* FIXME */break;
      case 0xe9:/*SBC #*/_A_IMM();/* FIXME */break;
      case 0xea:/*NOP */_A_IMP();_RD();break;
      case 0xeb:/*SBC # (undoc)*/_A_IMM();/* FIXME */break;
      case 0xec:/*CPX abs*/_A_ABS();/* FIXME */break;
      case 0xed:/*SBC abs*/_A_ABS();/* FIXME */break;
      case 0xee:/*INC abs*/_A_ABS();/* FIXME */break;
      case 0xef:/*ISB abs (undoc)*/_A_ABS();/* FIXME */break;
      case 0xf0:/*BEQ #*/_A_IMM();/* FIXME */break;
      case 0xf1:/*SBC (zp),Y*/_A_IDY_R();/* FIXME */break;
      case 0xf2:/*INC *//* FIXME */break;
      case 0xf3:/*ISB (zp),Y (undoc)*/_A_IDY_W();/* FIXME */break;
      case 0xf4:/*NOP zp,X (undoc)*/_A_ZPX();_RD();break;
      case 0xf5:/*SBC zp,X*/_A_ZPX();/* FIXME */break;
      case 0xf6:/*INC zp,X*/_A_ZPX();/* FIXME */break;
      case 0xf7:/*ISB zp,X (undoc)*/_A_ZPX();/* FIXME */break;
      case 0xf8:/*SE DF */_A_IMP();/* FIXME */break;
      case 0xf9:/*SBC abs,Y*/_A_ABY_R();/* FIXME */break;
      case 0xfa:/*NOP  (undoc)*/_A_IMP();_RD();break;
      case 0xfb:/*ISB abs,Y (undoc)*/_A_ABY_W();/* FIXME */break;
      case 0xfc:/*NOP abs,X (undoc)*/_A_ABX_R();_RD();break;
      case 0xfd:/*SBC abs,X*/_A_ABX_R();/* FIXME */break;
      case 0xfe:/*INC abs,X*/_A_ABX_W();/* FIXME */break;
      case 0xff:/*ISB abs,X (undoc)*/_A_ABX_W();/* FIXME */break;
    }
    /* check for interrupt request */
    if ((pins & M6502_NMI) || ((pins & M6502_IRQ) && !(c.P & M6502_IF))) {
      /* execute a slightly modified BRK instruction */
      _RD();
      _SAD(0x0100|c.S--, c.PC>>8); _WR();
      _SAD(0x0100|c.S--, c.PC); _WR();
      _SAD(0x0100|c.S--, c.P&~M6502_BF); _WR();
      if (pins & M6502_NMI) {
        _SA(0xFFFA); _RD(); l=_GD();
        _SA(0xFFFB); _RD(); h=_GD();
      }
      else {
        _SA(0xFFFE); _RD(); l=_GD();
        _SA(0xFFFF); _RD(); h=_GD();
      }
      c.PC = (h<<8)|l;
      c.P |= M6502_IF;
      pins &= ~(M6502_IRQ|M6502_NMI);
    }
  } while ((ticks < num_ticks) && ((pins & c.break_mask)==0));
  c.PINS = pins;
  *cpu = c;
  return ticks;
}
#undef _SA
#undef _SAD
#undef _GD
#undef _ON
#undef _OFF
#undef _T
#undef _RD
#undef _WR
#undef _A_IMP
#undef _A_IMM
#undef _A_ZER
#undef _A_ZPX
#undef _A_ZPY
#undef _A_ABS
#undef _A_ABX_R
#undef _A_ABX_W
#undef _A_ABY_R
#undef _A_ABY_W
#undef _A_IDX
#undef _A_IDY_R
#undef _A_IDY_W
