...
static uint8_t _ppu_readPalette(uint8_t address, void* user_data);
static void _ppu_write(uint16_t address, uint8_t data, void* user_data);

static uint8_t _ppu_read(uint16_t addr, void* user_data) {
    nes_t* sys = (nes_t*)user_data;
    CHIPS_ASSERT(sys && sys->valid);
    if (addr < 0x2000 && sys->mapper.read_chr) {
        return sys->mapper.read_chr(addr, sys->mapper.user_data);
    }
    if (addr < 0x3eff) {
    ...
    if (addr < 0x3fff) {
        ...
        return _ppu_readPalette(paletteAddr, user_data);
    }
    return 0;
}
...
static void _ppu_scanlineIRQ(void* user_data) {
    (void)user_data;
}
...
static void _ppu_vblankCallback(void* user_data) {
    nes_t* sys = (nes_t*)user_data;
    CHIPS_ASSERT(sys && sys->valid);
    sys->pins |= M6502_NMI;
}

static void _ppu_setPixel(int x, int y, uint8_t color, void* user_data) {
    nes_t* sys = (nes_t*)user_data;
    CHIPS_ASSERT(sys && sys->valid);
...
void nes_init(nes_t* sys, const nes_desc_t* desc) {
    ...
    r2c02_init(&sys->ppu, &(r2c02_desc_t){
        .read = _ppu_read,
        .write = _ppu_write,
        .scanlineIRQ = _ppu_scanlineIRQ,
        .readPalette = _ppu_readPalette,
        .vblankCallback = _ppu_vblankCallback,
        .setPixel = _ppu_setPixel,
        .user_data = sys,
    });

    mem_init(&sys->mem);
    mem_map_ram(&sys->mem, 0, 0x0000, 2048, sys->ram);
        ...
        if (pins & M6502_RW) {
            ...
            if(addr < 0x2000) {
                ...
                if(addr == _PPUCTRL) {
                    ...
                    sys->ppu.spriteDataAddress = data;
                } else if(addr == _PPUADDR) {
                    sys->ppu.spriteMemory[addr] = data;
                    ...
                    sys->ppu.spriteMemory[sys->ppu.spriteDataAddress++] = data;
                }
            }
        }
    }
