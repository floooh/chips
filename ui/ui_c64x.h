...
/*#
    ...
    Integrated debugging UI for c64x.h
...
/* reboot callback */
typedef void (*ui_c64x_boot_cb)(c64_t* sys);

/* setup params for ui_c64_init() */
typedef struct {
    c64_t* c64;             /* pointer to c64_t instance to track */
    ui_c64x_boot_cb boot_cb; /* reboot callback function */
...
static void _ui_c64x_draw_menu(ui_c64x_t* ui, double time_ms) {
...
static const char* _ui_c64x_memlayer_names[_UI_C64_MEMLAYER_NUM] = {
    "CPU Mapped", "RAM Banks", "ROM Banks", "VIC Mapped", "Color RAM"
};

static uint8_t _ui_c64x_mem_read(int layer, uint16_t addr, void* user_data) {
    CHIPS_ASSERT(user_data);
    ui_c64x_t* ui = (ui_c64x_t*) user_data;
    c64_t* c64 = ui->c64;
    switch (layer) {
...
static void _ui_c64x_mem_write(int layer, uint16_t addr, uint8_t data, void* user_data) {
    CHIPS_ASSERT(user_data);
    ui_c64x_t* ui = (ui_c64x_t*) user_data;
    c64_t* c64 = ui->c64;
    switch (layer) {
...
static void _ui_c64x_update_memmap(ui_c64x_t* ui) {
...
static int _ui_c64x_eval_bp(ui_dbg_t* dbg_win, uint16_t pc, int ticks, uint64_t pins, void* user_data) {
    CHIPS_ASSERT(user_data);
    ui_c64x_t* ui = (ui_c64x_t*) user_data;
    c64_t* c64 = ui->c64;
...
static const ui_chip_pin_t _ui_c64x_sid_pins[] = {
    { "D0",     0,      M6581_D0 },
    { "D1",     1,      M6581_D1 },
    { "D2",     2,      M6581_D2 },
    { "D3",     3,      M6581_D3 },
...
static const ui_chip_pin_t _ui_c64x_vic_pins[] = {
    { "DB0",    0,      M6569_D0 },
    { "DB1",    1,      M6569_D1 },
    { "DB2",    2,      M6569_D2 },
    { "DB3",    3,      M6569_D3 },
...
void ui_c64x_init(ui_c64x_t* ui, const ui_c64x_desc_t* ui_desc) {
    ...
    int x = 20, y = 20, dx = 10, dy = 10;
    {
        ...
        desc.read_cb = _ui_c64x_mem_read;
        desc.break_cb = _ui_c64x_eval_bp;
    ...
    x += dx; y += dy;
    {
        ...
        desc.x = x;
        desc.y = y;
        UI_CHIP_INIT_DESC(&desc.chip_desc, "6526", 40, _ui_c64x_cia_pins);
    ...
    x += dx; y += dy;
    {
        ...
        desc.x = x;
        desc.y = y;
        UI_CHIP_INIT_DESC(&desc.chip_desc, "6581", 16, _ui_c64x_sid_pins);
    ...
    x += dx; y += dy;
    {
        ...
        desc.x = x;
        desc.y = y;
        UI_CHIP_INIT_DESC(&desc.chip_desc, "6569", 28, _ui_c64x_vic_pins);
    ...
    x += dx; y += dy;
    {
        ...
        for (int i = 0; i < _UI_C64_MEMLAYER_NUM; i++) {
            desc.layers[i] = _ui_c64x_memlayer_names[i];
        }
        desc.read_cb = _ui_c64x_mem_read;
        desc.write_cb = _ui_c64x_mem_write;
        desc.user_data = ui;
...
void ui_c64x_draw(ui_c64x_t* ui, double time_ms) {
    CHIPS_ASSERT(ui && ui->c64);
    _ui_c64x_draw_menu(ui, time_ms);
    if (ui->memmap.open) {
        _ui_c64x_update_memmap(ui);
    }
