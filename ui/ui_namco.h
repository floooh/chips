...
static uint8_t _ui_namco_mem_read(int layer, uint16_t addr, void* user_data) {
    ...
    switch (layer) {
        ...
        case _UI_NAMCO_MEMLAYER_SOUND:
            return (addr < sizeof(ui->sys->rom_sound)) ? ui->sys->rom_sound[addr/0x0100][addr&0x00FF] : 0xFF;
        default:
            return 0xFF;
    }
}
...
static void _ui_namco_mem_write(int layer, uint16_t addr, uint8_t data, void* user_data) {
    ...
    switch (layer) {
        ...
        case _UI_NAMCO_MEMLAYER_PROM:
            ...
            break;
        case _UI_NAMCO_MEMLAYER_SOUND:
            if (addr < sizeof(ui->sys->rom_sound)) {
                ui->sys->rom_sound[addr/0x0100][addr&0x00FF] = data;
            }
            break;
        default:
            break;
