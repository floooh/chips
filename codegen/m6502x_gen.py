...
#-------------------------------------------------------------------------------
def _asl(val):
    s  = 'c->P=(c->P&~M6502X_CF)|(('+val+'&0x80)?M6502X_CF:0);'
    s += val+'<<=1;'
    s += '_NZ('+val+');'
    return s
...
#-------------------------------------------------------------------------------
def _lsr(val):
    s  = 'c->P=(c->P&~M6502X_CF)|(('+val+'&0x01)?M6502X_CF:0);'
    s += val+'>>=1;'
    s += '_NZ('+val+');'
    return s
...
def _rol(val):
    s  = '{'
    s += 'bool carry=c->P&M6502&_CF;'
    s += 'c->P&=~(M6502X_NF|M6502Z_ZF|M6502Z_CF);'
    s += 'if('+val+'&0x80){c->P|=M6502X_CF;}'
    s += val+'<<=1;'
    s += 'if(carry){'+val+'|=0x01;}'
    s += '_NZ('+val+');'
    s += '}'
