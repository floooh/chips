...
/*#
    ...
    ~~~C
    uint16_t z80dasm_op(uint16_t pc, z80dasm_in_t in_fn, void* in_userdata, z80dasm_out_t out_fn, void* out_userdata);
    ~~~

    pc      - the current 16-bit program counter, this is used to compute
              absolute target addresses for relative jumps
    in_fn           - this function is called when the disassembler needs the next
                      instruction byte: uint8_t in_fn(void* userdata)
    in_userdata     - a user-provided context pointer for the in_fn callback
    out_fn          - (optional) this function is called when the disassembler produces a single
                      ASCII character: void out_fn(char c, void* userdata)
    out_userdata    - a user-provided context pointer for the out_fn callback
...
typedef uint8_t (*z80dasm_in_t)(void* userdata);
// the output callback type
typedef void (*z80dasm_out_t)(char c, void* userdata);
...
#undef _FETCH_U8
#endif
#define _FETCH_U8(v) v=in_fn(in_fn_ud);pc++;
// fetch signed 8-bit value and track pc
#ifdef _FETCH_I8
#undef _FETCH_I8
#endif
#define _FETCH_I8(v) v=(int8_t)in_fn(in_fn_ud);pc++;
// fetch unsigned 16-bit value and track pc
#ifdef _FETCH_U16
#undef _FETCH_U16
#endif
#define _FETCH_U16(v) v=in_fn(in_fn_ud);v|=in_fn(in_fn_ud)<<8;pc+=2;
// output character
#ifdef _CHR
#undef _CHR
#endif
#define _CHR(c) if (out_fn) { out_fn(c,out_fn_ud); }
// output string
#ifdef _STR
#undef _STR
#endif
#define _STR(s) _z80dasm_str(s,out_fn,out_fn_ud);
// output offset as signed 8-bit string (decimal)
#ifdef _STR_D8
#undef _STR_D8
#endif
#define _STR_D8(d8) _z80dasm_d8((int8_t)(d8),out_fn,out_fn_ud);
// output number as unsigned 8-bit string (hex)
#ifdef _STR_U8
#undef _STR_U8
#endif
#define _STR_U8(u8) _z80dasm_u8((uint8_t)(u8),out_fn,out_fn_ud);
// output number number as unsigned 16-bit string (hex)
#ifdef _STR_U16
#undef _STR_U16
#endif
#define _STR_U16(u16) _z80dasm_u16((uint16_t)(u16),out_fn,out_fn_ud);
// (HL)/(IX+d)/(IX+d)
#ifdef _M
#undef _M
#endif
...
// output a string
static void _z80dasm_str(const char* str, z80dasm_out_t out_fn, void* out_fn_ud) {
    if (out_fn) {
        char c;
        while (0 != (c = *str++)) {
            out_fn(c, out_fn_ud);
...
static void _z80dasm_d8(int8_t val, z80dasm_out_t out_fn, void* out_fn_ud) {
    if (out_fn) {
        if (val < 0) {
            out_fn('-', out_fn_ud);
            val = -val;
        ...
        if ((val/10) != 0) {
            out_fn(_z80dasm_dec[val/10], out_fn_ud);
        }
        out_fn(_z80dasm_dec[val%10], out_fn_ud);
    }
}
...
static void _z80dasm_u8(uint8_t val, z80dasm_out_t out_fn, void* out_fn_ud) {
    if (out_fn) {
        for (int i = 1; i >= 0; i--) {
            out_fn(_z80dasm_hex[(val>>(i*4)) & 0xF], out_fn_ud);
        }
        out_fn('h',out_fn_ud);
    }
}
...
static void _z80dasm_u16(uint16_t val, z80dasm_out_t out_fn, void* out_fn_ud) {
    if (out_fn) {
        for (int i = 3; i >= 0; i--) {
            out_fn(_z80dasm_hex[(val>>(i*4)) & 0xF], out_fn_ud);
        }
...
uint16_t z80dasm_op(uint16_t pc, z80dasm_in_t in_fn, void* in_fn_ud, z80dasm_out_t out_fn, void* out_fn_ud) {
    CHIPS_ASSERT(in_fn);
